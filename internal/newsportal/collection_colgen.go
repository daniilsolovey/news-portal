...
func NewCategories(in []db.Category) Categories { return Map(in, NewCategory) }
...
func NewTags(in []db.Tag) Tags { return Map(in, NewTag) }

func (ll Tags) IndexByID() map[int]Tag {
	r := make(map[int]Tag, len(ll))
	for i := range ll {
		r[ll[i].ID] = ll[i]
	}
